...
module.exports.verifyPatreon = function (app) {
  return async function (req, res, next) {
    ...
    const userPatreonObject = await app
      .service("users")
      .get(user.id)
      .then((data) => {
        return data.patreon;
    ...
    if (!patronData) {
      return res.json({
        error: true,
        errorMsg: "You are currently not a patron",
      });
    }
    ...
    if (amount < 500) {
      return res.json({
        error: true,
        errorMsg: "Patron status is only allowed at $5 or more",
      });
    ...
    if (last_charged_status !== "paid") {
      return res.json({
        error: true,
        errorMsg: "Last patreon payment was declined",
      });
    ...
    app
      ...
      .catch((e) => {
        ...
        return res.json({
          error: true,
          errorMsg: "An error occurred while linking your account!",
        });
      });
  };
};
