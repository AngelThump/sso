...
const patreon = require('../../config/patreon.json');
...
module.exports.verifyPatreon = function (app) {
  return async function (req, res, next) {
    if(!req.user) return res.status(500).json({error: true, message: "No user"}); 

    const user = req.user;

    if(!user.patreon) return res.status(500).json({error: true, message: "User is not connected with Patreon"}); 
    ...
    if(!isValid) {
      ...
      if(!tokens) return res.status(500).json({error: true, message: "Server encountered an error"});
    ...
    if(!membership_id) return res.status(404).json({error: true, message: "You are not a patron"});
    ...
    if(!tiers) return res.status(500).json({error: true, message: "Server encountered an error"});
    ...
    if(user.patreon.isPatron && user.patreon.tier === newTier) return res.json({ error: true, message: "You are a patron already!" });
...
const getTiers = async() => {
  let tiers;
  await axios.get(`https://www.patreon.com/api/oauth2/v2/campaigns/${patreon.campaignID}?include=tiers&fields%5Btier%5D=amount_cents,title`, {
    headers: {
...
const checkUserToken = async (access_token) => {
  let isValid;
  await axios.get(`https://www.patreon.com/api/oauth2/v2/identity?include=memberships`, {
    headers: {
...
const getMembershipId = async (access_token) => {
  let membership_id;
  await axios.get(`https://www.patreon.com/api/oauth2/v2/identity?include=memberships,memberships.campaign`, {
    headers: {
      Authorization: `Bearer ${access_token}`,
    }
  })
  .then(response => {
    if (!response.data.included && typeof response.data.included[Symbol.iterator] !== "function") return;
...
const getPatronData = async (patronId) => {
  let patronData;
  await axios.get(`https://www.patreon.com/api/oauth2/v2/members/${patronId}?include=currently_entitled_tiers,user&fields%5Bmember%5D=patron_status,email,pledge_relationship_start,campaign_lifetime_support_cents,currently_entitled_amount_cents,last_charge_date,last_charge_status,will_pay_amount_cents`, {
    headers: {
      Authorization: `Bearer ${patreon.CREATOR_ACCESS_TOKEN}`,
    }
  })
...
const refreshCreatorToken = async() => {
  ...
  .then(response => {
    ...
    fs.writeFile(path.resolve(__dirname, "../../config/patreon.json"), JSON.stringify(patreon, null, 4), (err) => {
...
const checkCreatorToken = async () => {
  let isValid;
  await axios.get(`https://www.patreon.com/api/oauth2/v2/campaigns`, {
    headers: {
      Authorization: `Bearer ${patreon.CREATOR_ACCESS_TOKEN}`,
    }
  })
